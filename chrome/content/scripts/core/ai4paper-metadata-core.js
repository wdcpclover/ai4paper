...
var AI4PaperMetadataCore = (() => {
    function readMetadataTemplate(templateStr, marker) {
    ...
    function normalizeInlineMetadataText(text) {
        return String(text ?? "")
            .replace(/\:/g, "：")
            .replace(/： /g, "：")
            .replace(/\"/g, "\u201C")
    ...
    function normalizeAbstractYamlText(text) {
        return String(text ?? "")
            .replace(/\n\n【摘要翻译】/g, "【摘要翻译】")
            .replace(/\:/g, "：")
            .replace(/： /g, "：")
    ...
    function sanitizeQNKeyText(text) {
        return String(text ?? "")
            .trim()
            .replace(/\\/g, " ")
            .replace(/\//g, " ")
    ...
    function buildDefaultQNKey(input) {
        const maxLenZh = 10;
        const maxLenEn = 20;
    ...
    function parseTruncateValue(value) {
        const parsed = Number.parseInt(String(value ?? ""), 10);
        return Number.isNaN(parsed) ? null : parsed;
    }
    function applyAffixes(value, config) {
        return `${config?.prefix || ""}${value}${config?.suffix || ""}`;
    }
    function sanitizeQNKeyVariableValue(value) {
        return sanitizeQNKeyText(value);
    }
    function resolveQNKeyVariable(item, config, varName) {
        try {
    ...
    function findTemplateDelimiterPositions(templateStr, isClose) {
    ...
    function resolveQNKeyTemplate(templateStr, item) {
        ...
        for (let i = 0; i < openPositions.length; i++) {
            try {
                const config = JSON.parse(normalizedTemplate.substring(openPositions[i] + 3, closePositions[i]).trim());
                if (config.variable) {
    ...
    function formatMetadataCreators(input) {
    ...
    function normalizeTemplateReplacementValue(value) {
        return String(value ?? "");
    }
    function buildMetadataTemplateValues(input) {
    ...
    function applyMetadataTemplate(template, input) {
    ...
    function buildSimpleTemplateValues(input) {
        return {
            itemType: normalizeTemplateReplacementValue(input.itemType),
            title: normalizeTemplateReplacementValue(input.title),
            shortTitle: normalizeTemplateReplacementValue(input.shortTitle).replace(/ /g, ""),
    ...
    function applySimpleTemplate(template, input) {
        let result = String(template ?? "");
    ...
    function formatYamlCreators(title, creators) {
    ...
    function formatFirstCreator(title, creator) {
        const firstName = String(creator?.firstName ?? "");
        const lastName = String(creator?.lastName ?? "");
    ...
    function getNormalizedJournalAbbreviation(title, journalAbbreviation, publicationTitle) {
    ...
    function normalizeYamlAbstractText(text) {
        return String(text ?? "")
            .replace(/\n\n【摘要翻译】/g, "【摘要翻译】")
            .replace(/\:/g, "：")
            .replace(/： /g, "：")
    ...
    function buildYamlTemplateValues(input) {
        return {
            itemType: normalizeTemplateReplacementValue(input.itemType),
            title: normalizeTemplateReplacementValue(input.title),
            shortTitle: normalizeTemplateReplacementValue(input.shortTitle).replace(/ /g, ""),
    ...
    function applyYamlTemplate(template, input) {
        let result = String(template ?? "");
    ...
    function buildMetadataCreatorUpdates(firstNames, lastNames) {
    ...
    function buildMetadataUpdatePlan(target, prefs, options) {
        const fieldUpdates = [];
        ...
        const creatorUpdates = prefs.authors
            ...
            : [];
        return {
            hasAnyData: target._Data_title != null ||
