function init() {
  ...
  Zotero.ZoteroIF.gptReaderSidePane_setStickyScroll(document, true);
  injectRenderingFunc();
  Zotero.ZoteroIF.gptReaderSidePane_injectStyle(window);
  Zotero.ZoteroIF.updateSlogan4ChatUI(window);
  Zotero.ZoteroIF.updateChatUI4HighlightStyle(window);
  document.body.style.margin = 0x0;
  document.body.style.padding = 0x0;
  Zotero.ZoteroIF.setOptions4marked();
...
function setUIHeight() {
  ...
  if (!Zotero.Prefs.get("zoteroif.gptEnableCustomChatModeGPTUIHeight")) {
    let _0x1cb755, _0x5a7959;
    parseInt(var3) <= 0x3e8 ? (_0x1cb755 = var3 * 0.185, _0x5a7959 = var3 * 0.446, var1.style.height = _0x1cb755 + 'px', var2.style.height = _0x5a7959 + 'px', Zotero.Prefs.set("zoteroif.gptCustomChatModePromptAreaHeight", _0x1cb755.toFixed(0x2)), Zotero.Prefs.set("zoteroif.gptCustomChatModeResponseAreaHeight", _0x5a7959.toFixed(0x2))) : (_0x1cb755 = var3 * 0.173, _0x5a7959 = var3 * 0.555, var1.style.height = _0x1cb755 + 'px', var2.style.height = _0x5a7959 + 'px', Zotero.Prefs.set("zoteroif.gptCustomChatModePromptAreaHeight", _0x1cb755.toFixed(0x2)), Zotero.Prefs.set('zoteroif.gptCustomChatModeResponseAreaHeight', _0x5a7959.toFixed(0x2)));
    Zotero.ZoteroIF.gptReaderSidePane_setUIHeight(_0x1cb755, _0x5a7959);
  } else {
    var1.style.height = Zotero.Prefs.get("zoteroif.gptCustomChatModePromptAreaHeight") + 'px';
    var2.style.height = Zotero.Prefs.get("zoteroif.gptCustomChatModeResponseAreaHeight") + 'px';
    Zotero.ZoteroIF.gptReaderSidePane_setUIHeight(Zotero.Prefs.get("zoteroif.gptCustomChatModePromptAreaHeight"), Zotero.Prefs.get("zoteroif.gptCustomChatModeResponseAreaHeight"));
  }
}
function addRightClickEvent_messageInput() {
  ...
  var6.addEventListener("mousedown", _0x1eeaa5 => {
    if (_0x1eeaa5.altKey) {
      let _0x28bdf5 = Zotero.ZoteroIF._calculateTokens(var6.value.trim()).length;
      Services.prompt.alert(window, "✨ tokens 计算", "输入框中文字的 tokens 计算值为：" + _0x28bdf5);
      return;
    }
    _0x1eeaa5.button == 0x2 && Zotero.ZoteroIF.gptReaderSidePane_clearPrompt();
  });
}
function setContextMenu() {
  ...
  for (var var11 = 0x0; var11 < var10.length; var11++) {
    if (var11 === 0x0) var10[var11].addEventListener('click', function () {
      var9.style.display = "none";
      Zotero.ZoteroIF.gptReaderSidePane_ChatMode_onClickMenuItemSelectMessage();
    });else {
      if (var11 === 0x1) var10[var11].addEventListener('click', function () {
        var9.style.display = "none";
        Zotero.ZoteroIF.gptReaderSidePane_ChatMode_checkAllMessages();
        Zotero.Prefs.set("zoteroif.gptUnderSelectionMessage", true);
      });else {
        if (var11 === 0x2) {
          var10[var11].addEventListener("click", function () {
            var9.style.display = "none";
            Zotero.ZoteroIF.gptReaderSidePane_ChatMode_showMessageCheckbox();
          });
        } else {
          if (var11 === 0x3) var10[var11].addEventListener("click", function () {
            var9.style.display = "none";
            Zotero.ZoteroIF.gptReaderSidePane_ChatMode_setMessageAsTranslationSourceText(Zotero.ZoteroIF._contextmenuEvent_messageEl);
          });else {
            if (var11 === 0x4) var10[var11].addEventListener("click", function () {
              var9.style.display = "none";
              Zotero.ZoteroIF.gptReaderSidePane_ChatMode_copySelectedMessages();
            });else {
              if (var11 === 0x5) var10[var11].addEventListener("click", function () {
                var9.style.display = "none";
                Zotero.ZoteroIF.gptReaderSidePane_ChatMode_copyAssistantMessages();
              });else {
                if (var11 === 0x6) var10[var11].addEventListener("click", function () {
                  var9.style.display = "none";
                  Zotero.ZoteroIF.gptReaderSidePane_ChatMode_go2MessageTop(Zotero.ZoteroIF._contextmenuEvent_messageEl);
                });else {
                  if (var11 === 0x7) var10[var11].addEventListener("click", function () {
                    var9.style.display = "none";
                    Zotero.ZoteroIF.gptReaderSidePane_ChatMode_createAIReadingNoteItem(Zotero.ZoteroIF._contextmenuEvent_messageEl, false);
                  });else {
                    if (var11 === 0x8) var10[var11].addEventListener("click", function () {
                      var9.style.display = "none";
                      Zotero.ZoteroIF.gptReaderSidePane_ChatMode_createAIReadingNoteItem(Zotero.ZoteroIF._contextmenuEvent_messageEl, true);
                    });else {
                      if (var11 === 0x9) var10[var11].addEventListener('click', function () {
                        var9.style.display = 'none';
                        Zotero.ZoteroIF.gptReaderSidePane_ChatMode_copyMessageSourceText(Zotero.ZoteroIF._contextmenuEvent_messageEl);
                      });else {
                        if (var11 === 0xa) {
                          var10[var11].addEventListener("click", function () {
                            var9.style.display = 'none';
                            Zotero.ZoteroIF.gptReaderSidePane_ChatMode_copyMessage(Zotero.ZoteroIF._contextmenuEvent_messageEl);
                          });
                        } else {
                          if (var11 === 0xb) var10[var11].addEventListener('click', function () {
                            var9.style.display = "none";
                            Zotero.ZoteroIF.gptReaderSidePane_ChatMode_copyFileName(Zotero.ZoteroIF._contextmenuEvent_messageEl);
                          });else {
                            if (var11 === 0xc) var10[var11].addEventListener("click", function () {
                              var9.style.display = "none";
                              Zotero.ZoteroIF.gptReaderSidePane_ChatMode_copyFileFullText(Zotero.ZoteroIF._contextmenuEvent_messageEl);
                            });else {
                              if (var11 === 0xd) var10[var11].addEventListener("click", function () {
                                var9.style.display = "none";
                                Zotero.ZoteroIF.gptReaderSidePane_ChatMode_openFileInZotero(Zotero.ZoteroIF._contextmenuEvent_messageEl);
                              });else var11 === 0xe && var10[var11].addEventListener('click', function () {
                                var9.style.display = "none";
                                Zotero.ZoteroIF.gptReaderSidePane_ChatMode_showFile(Zotero.ZoteroIF._contextmenuEvent_messageEl);
                              });
                            }
                          }
                        }
...
function pushText() {
  Zotero.Prefs.set("zoteroif.chatgptprompt", document.getElementById("message-input").value);
}
function messageInput_checkKeyEnter(param2) {
  if (!param2.shiftKey && !param2.ctrlKey && !param2.altKey && !param2.metaKey && param2.keyCode === 0xd) {
    ...
    Zotero.ZoteroIF.gptReaderSidePane_ChatMode_send();
...
function metaso() {
  ...
  if (!var12) return;
  Zotero.ZoteroIF.meataso_WebSearch(var12);
}
function paperAI(param3) {
  if (param3.shiftKey) {
    Zotero.ZoteroIF._notClearChatOnPaperAI = true;
    Zotero.ZoteroIF.paperAI(false);
  } else {
    if (param3.button === 0x0) Zotero.ZoteroIF.paperAI(false);else param3.button === 0x2 && Zotero.ZoteroIF.paperAI(true);
  }
}
function autoResize() {
...
function injectRenderingFunc() {
  try {
    !Zotero.ZoteroIF.renderMarkdownLaTeX && (Zotero.ZoteroIF.renderMarkdownLaTeX = function (param4) {
      try {
    ...
    !Zotero.ZoteroIF.renderMarkdown && (Zotero.ZoteroIF.renderMarkdown = function (param7) {
      try {
    ...
    if (!Zotero.ZoteroIF.highlightAll) {
      Zotero.ZoteroIF.highlightAll = function () {
        hljs.highlightAll();
      };
    }
  } catch (_0x23ad72) {
...
function onClickPaperAIBatchBtn(param11) {
  if (param11.shiftKey) {
    Zotero.ZoteroIF.openDialogByType("batchAIInterpretSettings");
    return;
  }
  Zotero.ZoteroIF.openDialogByType("batchAIInterpret", true);
}
async function onClickChatHistoryBtn(param12) {
  if (param12.shiftKey) {
    let var18 = Zotero.ZoteroIF.getCurrentReader();
    if (var18) {
      let _0x25ac8 = var18._item.key,
        _0x3b9820 = await hasTargetChatHistory(_0x25ac8);
      if (!_0x3b9820) {
        Zotero.ZoteroIF.showProgressWindow(0xbb8, '❌\x20未找到目标对话【Zotero\x20One】', "未在电脑本地找到当前附件的【文献解读】对话历史。👉 请确认设置过 AI 对话历史本地存储文件夹。");
        return;
      }
      let _0x114bb9 = await selectTargetItem(_0x25ac8);
      if (!_0x114bb9) {
        Zotero.ZoteroIF.showProgressWindow(0xbb8, "❌ 未找到目标对话【AI4paper】", "未在【AI 对话历史】窗口找到当前附件的【文献解读】对话历史。👉 请确认设置过 AI 对话历史本地存储文件夹。");
      }
    }
    return;
  }
  Zotero.ZoteroIF.openDialogByType("chatHistory", true);
}
async function hasTargetChatHistory(param13) {
  let var22 = Zotero.Prefs.get("zoteroif.gptChatHistoryLocalPath"),
    var23 = await Zotero.ZoteroIF.readChatHistoryFromLocal(var22);
...
async function selectTargetItem(param14) {
  let var27 = Zotero.ZoteroIF.openDialogByType("chatHistory", true);
  if (!var27) return;
