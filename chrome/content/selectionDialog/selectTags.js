...
methodsBody.init = async function () {
  ...
  Zotero.ZoteroIF.lasttaginput && (document.getElementById('searchBox-elem').placeholder = Zotero.ZoteroIF.lasttaginput);
  ...
  let var1,
    var2 = Zotero.ZoteroIF.lastTagsSelectPane;
  if (!methodsBody._is4AnnotationTags) {
    document.title = "添加条目标签";
    Zotero.getMainWindow().Zotero_Tabs._selectedID === "zotero-pane" && (document.title = '向【' + Zotero.ZoteroIF._data_selectedItemsNum + '】个条目添加条目标签');
    if (var2 === 'itemTagsPane') {
      var1 = Zotero.ZoteroIF.returnItemTags();
      methodsBody.updateTagsNumMessage(var1, 'itemTag');
      methodsBody.updateButtonStatus("itemTag");
    } else var2 === "recentItemTagsPane" ? (var1 = Zotero.Prefs.get("zoteroif.recentlyAddedItemTags").split("😊🎈🍓"), var1.length === 0x1 && var1[0x0] === '' && (var1 = []), methodsBody.updateTagsNumMessage(var1, "recentItemTag"), methodsBody.updateButtonStatus("recentItemTag")) : (var1 = Zotero.ZoteroIF.returnItemTags(), methodsBody.updateTagsNumMessage(var1, "itemTag"), methodsBody.updateButtonStatus("itemTag"));
  } else {
    if (var2 === "annotationTagsPane") {
      var1 = Zotero.ZoteroIF.returnAnnotationTags();
      ...
      if (var2 === "imageAnnotationTagsPane") {
        var1 = Zotero.ZoteroIF.returnImageAnnotationTags();
        methodsBody.updateTagsNumMessage(var1, "imageAnnotationTag");
        methodsBody.updateButtonStatus("imageAnnotationTag");
      } else var2 === 'recentTagsPane' ? (var1 = Zotero.Prefs.get('zoteroif.recentlyaddedTags').split("😊🎈🍓"), var1.length === 0x1 && var1[0x0] === '' && (var1 = []), methodsBody.updateTagsNumMessage(var1, "recentTag"), methodsBody.updateButtonStatus('recentTag')) : (var1 = Zotero.ZoteroIF.returnAnnotationTags(), methodsBody.updateTagsNumMessage(var1, "annotationTag"), methodsBody.updateButtonStatus("annotationTag"));
    }
  }
...
methodsBody.imageAnnotationTag = function (param3) {
  methodsBody.updateButtonStatus('imageAnnotationTag');
  let var5 = Zotero.ZoteroIF.returnImageAnnotationTags();
  methodsBody.clearListbox();
...
methodsBody.filter = function (param4) {
  ...
  if (var7.getAttribute("src") === "chrome://zoteroif/content/icons/annotationTag.png") {
    var6 = Zotero.ZoteroIF.returnAnnotationTagsFilter(param4);
    methodsBody.updateButtonStatus("annotationTag");
  } else {
    if (var7.getAttribute("src") === "chrome://zoteroif/content/icons/imageAnnotationTag.png") {
      var6 = Zotero.ZoteroIF.returnImageAnnotationTagsFilter(param4);
      methodsBody.updateButtonStatus("imageAnnotationTag");
    } else var7.getAttribute('src') === "chrome://zoteroif/content/icons/itemTag.png" || !methodsBody._is4AnnotationTags && var7.getAttribute('src') === "chrome://zoteroif/content/icons/recentTag.png" ? (var6 = Zotero.ZoteroIF.returnItemTagsFilter(param4), methodsBody.updateButtonStatus("itemTag")) : (var6 = Zotero.ZoteroIF.returnAnnotationTagsFilter(param4), methodsBody.updateButtonStatus("annotationTag"));
  }
  methodsBody.clearListbox();
  methodsBody.buildItemNodes(var6);
  param4 = param4 === 'OT' ? '#' : param4;
  var var8 = document.getElementById("message-label");
  if (var7.getAttribute('src') === "chrome://zoteroif/content/icons/annotationTag.png") var8.textContent = param4 + "：包含【" + var6.length + "】个注释标签";else {
    if (var7.getAttribute('src') === "chrome://zoteroif/content/icons/imageAnnotationTag.png") var8.textContent = param4 + '：包含【' + var6.length + "】个图片注释标签";else var7.getAttribute("src") === "chrome://zoteroif/content/icons/itemTag.png" || !methodsBody._is4AnnotationTags && var7.getAttribute("src") === 'chrome://zoteroif/content/icons/recentTag.png' ? var8.textContent = param4 + "：包含【" + var6.length + "】个条目标签" : (var7.setAttribute("src", "chrome://zoteroif/content/icons/annotationTag.png"), var8.textContent = param4 + "：包含【" + var6.length + "】个注释标签");
  }
};
methodsBody.updateTags = async function (param5) {
  ...
  document.getElementById('annotationTagType-image').setAttribute("src", "chrome://zoteroif/content/icons/" + param5 + ".png");
  document.getElementById("message-label").textContent = "正在刷新" + var9[param5] + "，右下角查看进度...";
  let var11 = "_selectTagsDialog_update_" + param5;
  Zotero.ZoteroIF.progressPercent_initProgress(var10, var11, var9[param5]);
  methodsBody["update_" + param5 + "_checkNext"](var11, var9[param5]);
};
methodsBody.update_itemTag_checkNext = async function (param6, param7) {
  Zotero.ZoteroIF["numberOfUpdatedItems" + param6]++;
  if (Zotero.ZoteroIF['current' + param6] == Zotero.ZoteroIF['toUpdate' + param6] - 0x1) {
    Zotero.ZoteroIF["progressWindow" + param6].close();
    Zotero.ZoteroIF.progressPercent_resetState(null, param6, param7);
    Zotero.Prefs.set('zoteroif.itemTags', JSON.stringify(Zotero.ZoteroIF["_progressData_" + param6]));
    methodsBody.itemTag(true);
    return;
  }
  Zotero.ZoteroIF.progressPercent_updatePercent(param6, "检查所有标签： ");
  methodsBody.update_itemTag_checkTag(Zotero.ZoteroIF['itemsToUpdate' + param6][Zotero.ZoteroIF["current" + param6]], param6, param7);
};
methodsBody.update_itemTag_checkTag = async function (param8, param9, param10) {
  try {
    let var12 = await Zotero.ZoteroIF.checkItemTag(param8.tag);
    if (var12) {
      ...
      !JSON.stringify(Zotero.ZoteroIF["_progressData_" + param9]).includes(JSON.stringify(var15)) && (Zotero.ZoteroIF["_progressData_" + param9].push(var15), Zotero.ZoteroIF['counter' + param9]++);
    }
  } catch (_0x3dd5df) {
...
methodsBody.update_annotationTag_checkNext = async function (param11, param12) {
  Zotero.ZoteroIF["numberOfUpdatedItems" + param11]++;
  if (Zotero.ZoteroIF["current" + param11] == Zotero.ZoteroIF['toUpdate' + param11] - 0x1) {
    Zotero.ZoteroIF["progressWindow" + param11].close();
    Zotero.ZoteroIF.progressPercent_resetState(null, param11, param12);
    Zotero.Prefs.set("zoteroif.annotationtagsrecent", JSON.stringify(Zotero.ZoteroIF["_progressData_" + param11]));
    methodsBody.annotationTag(true);
    return;
  }
  Zotero.ZoteroIF.progressPercent_updatePercent(param11, "检查所有标签： ");
  methodsBody.update_annotationTag_checkTag(Zotero.ZoteroIF["itemsToUpdate" + param11][Zotero.ZoteroIF["current" + param11]], param11, param12);
};
methodsBody.update_annotationTag_checkTag = async function (param13, param14, param15) {
  try {
    let var16 = await Zotero.ZoteroIF.checkAnnotationTag(param13.tag);
    if (var16) {
      ...
      !JSON.stringify(Zotero.ZoteroIF['_progressData_' + param14]).includes(JSON.stringify(var19)) && (Zotero.ZoteroIF["_progressData_" + param14].push(var19), Zotero.ZoteroIF['counter' + param14]++);
    }
  } catch (_0x44fb7f) {
...
methodsBody.update_imageAnnotationTag_checkNext = async function (param16, param17) {
  Zotero.ZoteroIF["numberOfUpdatedItems" + param16]++;
  if (Zotero.ZoteroIF["current" + param16] == Zotero.ZoteroIF["toUpdate" + param16] - 0x1) {
    Zotero.ZoteroIF["progressWindow" + param16].close();
    Zotero.ZoteroIF.progressPercent_resetState(null, param16, param17);
    Zotero.Prefs.set("zoteroif.imageannotationtagsrecent", JSON.stringify(Zotero.ZoteroIF["_progressData_" + param16]));
    methodsBody.imageAnnotationTag(true);
    return;
  }
  Zotero.ZoteroIF.progressPercent_updatePercent(param16, "检查所有标签： ");
  methodsBody.update_imageAnnotationTag_checkTag(Zotero.ZoteroIF['itemsToUpdate' + param16][Zotero.ZoteroIF['current' + param16]], param16, param17);
};
methodsBody.update_imageAnnotationTag_checkTag = async function (param18, param19, param20) {
  try {
    let var20 = await Zotero.ZoteroIF.checkImageAnnotationTag(param18.tag);
    if (var20) {
      ...
      !JSON.stringify(Zotero.ZoteroIF["_progressData_" + param19]).includes(JSON.stringify(var23)) && (Zotero.ZoteroIF['_progressData_' + param19].push(var23), Zotero.ZoteroIF['counter' + param19]++);
    }
  } catch (_0x5b803f) {
...
methodsBody.recentItemTags = function () {
  ...
  let var24 = Zotero.Prefs.get("zoteroif.recentlyAddedItemTags").split("😊🎈🍓");
...
methodsBody.recentTags = function () {
  ...
  let var25 = Zotero.Prefs.get('zoteroif.recentlyaddedTags').split("😊🎈🍓");
...
methodsBody.search = function () {
  ...
  Zotero.ZoteroIF.lasttaginput = var26;
  let var27;
  var var28 = document.getElementById("annotationTagType-image");
  if (var28.getAttribute("src") === "chrome://zoteroif/content/icons/annotationTag.png") {
    var27 = Zotero.ZoteroIF.returnAnnotationTagsSearch(var26);
    methodsBody.updateButtonStatus('annotationTag');
  } else {
    if (var28.getAttribute("src") === "chrome://zoteroif/content/icons/imageAnnotationTag.png") {
      var27 = Zotero.ZoteroIF.returnImageAnnotationTagsSearch(var26);
      methodsBody.updateButtonStatus("imageAnnotationTag");
    } else var28.getAttribute("src") === "chrome://zoteroif/content/icons/itemTag.png" || !methodsBody._is4AnnotationTags && var28.getAttribute('src') === "chrome://zoteroif/content/icons/recentTag.png" ? (var27 = Zotero.ZoteroIF.returnItemTagsSearch(var26), methodsBody.updateButtonStatus('itemTag')) : (var27 = Zotero.ZoteroIF.returnAnnotationTagsSearch(var26), methodsBody.updateButtonStatus("annotationTag"));
  }
  methodsBody.clearListbox();
  methodsBody.buildItemNodes(var27);
  var var29 = document.getElementById("message-label");
  if (var28.getAttribute("src") === 'chrome://zoteroif/content/icons/annotationTag.png') var29.textContent = '搜索【' + var26 + '】：共有【' + var27.length + "】个注释标签";else {
    if (var28.getAttribute("src") === 'chrome://zoteroif/content/icons/imageAnnotationTag.png') var29.textContent = "搜索【" + var26 + "】：共有【" + var27.length + "】个图片注释标签";else var28.getAttribute("src") === "chrome://zoteroif/content/icons/itemTag.png" || !methodsBody._is4AnnotationTags && var28.getAttribute('src') === "chrome://zoteroif/content/icons/recentTag.png" ? var29.textContent = '搜索【' + var26 + "】：共有【" + var27.length + "】个条目标签" : (var28.setAttribute("src", "chrome://zoteroif/content/icons/annotationTag.png"), var29.textContent = "搜索【" + var26 + "】：共有【" + var27.length + "】个注释标签");
  }
  methodsBody.updateFilterButtons(true);
};
...
methodsBody.acceptSelection = function () {
  ...
  if (document.getElementById("selectedTags-inputBox-elem").value === '') return this.io.dataOut = null, false;else {
    ...
    if (var31.getAttribute("src") === 'chrome://zoteroif/content/icons/annotationTag.png') Zotero.ZoteroIF.lastTagsSelectPane = "annotationTagsPane";else {
      if (var31.getAttribute("src") === "chrome://zoteroif/content/icons/imageAnnotationTag.png") Zotero.ZoteroIF.lastTagsSelectPane = "imageAnnotationTagsPane";else {
        if (var31.getAttribute("src") === 'chrome://zoteroif/content/icons/itemTag.png') Zotero.ZoteroIF.lastTagsSelectPane = "itemTagsPane";else {
          if (!methodsBody._is4AnnotationTags && var31.getAttribute("src") === 'chrome://zoteroif/content/icons/recentTag.png') Zotero.ZoteroIF.lastTagsSelectPane = "recentItemTagsPane";else var31.getAttribute("src") === "chrome://zoteroif/content/icons/recentTag.png" ? Zotero.ZoteroIF.lastTagsSelectPane = "recentTagsPane" : Zotero.ZoteroIF.lastTagsSelectPane = "annotationTagsPane";
        }
      }
    }
  }
...
methodsBody.quickJump = function (param24) {
  if (!Zotero.Prefs.get("zoteroif.selectTagsDoubleClick")) return;
  let var44 = Zotero.Prefs.get("zoteroif.annotationtagsrecent"),
    var45 = Zotero.Prefs.get("zoteroif.imageannotationtagsrecent");
  ...
  if (document.getElementById('itemTag-button').getAttribute("default") === "true" || document.getElementById("recentItemTag-button").getAttribute("default") === "true") Zotero.ZoteroIF.queryPapersMatrix("filterByTag", param24);else {
    if (document.getElementById("imageAnnotationTag-button").getAttribute("default") === 'true') {
      ...
      Zotero.ZoteroIF.tagImageCardNotes(param24);
    } else document.getElementById("annotationTag-button").getAttribute("default") === 'true' || document.getElementById("recentTag-button").getAttribute("default") === "true" ? Zotero.Prefs.get('zoteroif.imageCardNoteSearchFirst') && var45.includes(JSON.stringify(var49)) ? (Zotero.Prefs.set("zoteroif.annotationtagtype", "type_imageAnnotationTag"), Zotero.ZoteroIF.tagImageCardNotes(param24)) : (Zotero.Prefs.set("zoteroif.annotationtagtype", "type_annotationTag"), Zotero.ZoteroIF.tagCardNotes(param24)) : (Zotero.Prefs.set("zoteroif.annotationtagtype", "type_annotationTag"), Zotero.ZoteroIF.tagCardNotes(param24));
  }
};
...
methodsBody.buildContextMenu = function (param26) {
  ...
  return var54.setAttribute("label", "拷贝标签"), var54.addEventListener("command", () => {
    ...
    Zotero.ZoteroIF.showProgressWindow(0x7d0, "拷贝标签【AI4paper】", '已拷贝标签【' + var51 + '】');
  ...
  for (var var56 in param27) {
    ...
    var60.addEventListener('click', _0x104659 => {
      ...
      if (_0x104659.shiftKey) {
        window.close();
        Zotero.ZoteroIF.showItemsBasedOnTag(_0x104659.target.closest("richlistitem")?.["querySelector"]("checkbox")["label"]);
        return;
      }
...
methodsBody.updateTagsNumMessage = function (param29, param30) {
  ...
  document.getElementById('annotationTagType-image').setAttribute("src", "chrome://zoteroif/content/icons/" + (param30 === 'recentItemTag' ? 'recentTag' : param30) + ".png");
};
methodsBody.updateButtonStatus = function (param31) {
  ...
  document.getElementById("annotationTagType-image").setAttribute("src", "chrome://zoteroif/content/icons/" + (param31 === 'recentItemTag' ? "recentTag" : param31) + ".png");
};
methodsBody.updateFilterButtons = function (param32, param33) {
  ...
  for (let var72 of var71) {
    let var73 = document.getElementById('tagFilter-' + var72),
      var74 = 'chrome://zoteroif/content/icons/' + var72 + '.png',
      var75 = "chrome://zoteroif/content/icons/" + var72 + '-select.png';
