function init() {
  ...
  Zotero.AI4Paper.gptNotesList_JS_injectStyle(window);
  ...
  setOptions4marked();
  let var3 = Zotero.AI4Paper._store_selecteGPTMessages;
...
function messageInput_checkKeyEnter(param1) {
  if (!param1.shiftKey && !param1.ctrlKey && !param1.altKey && !param1.metaKey && param1.keyCode === 0xd) {
    ...
    if (document.getElementById("finishEditing-button").style.display === "none") {
      Zotero.AI4Paper.showProgressWindow(0x7d0, "❌ 请先选择一个要编辑的消息【AI4paper】", "请先选择一个要编辑的消息，再执行此操作。");
      return;
    }
    finishEditMessage();
  }
...
function createMessageElement(param2, param3) {
  ...
  var9.innerHTML = Zotero.AI4Paper.svg_icon_20px.edit;
  var9.title = "点击编辑";
  var9.isEditing = false;
  var9.onclick = () => {
    if (!var9.isEditing) {
      var9.innerHTML = Zotero.AI4Paper.svg_icon_20px.editing;
      var9.title = "正在编辑中...";
      var9.isEditing = true;
  ...
  var11.innerHTML = param3 === 'user' ? Zotero.AI4Paper.svg_icon_20px.avatar_user : Zotero.AI4Paper.svg_icon_20px.gptviewReaderSidepane;
  const var12 = document.createElement("span");
  var12.classList.add('username');
  let var13 = Zotero.Prefs.get('ai4paper.gptUserName') ? Zotero.Prefs.get("ai4paper.gptUserName") : "User",
    var14 = Zotero.Prefs.get('ai4paper.gptservice') === "OpenAI" ? "ChatGPT" : Zotero.Prefs.get("ai4paper.gptservice");
  ...
  if (param3 === 'user') var15.innerText = param2;else {
    Zotero.AI4Paper.gptReaderSidePane_ChatMode_renderMessageContent(var15, param2);
  }
...
function setEditButtonsState(param4) {
  ...
  for (let var17 of var16) {
    if (var17 != param4) {
      var17.innerHTML = Zotero.AI4Paper.svg_icon_20px.edit;
      var17.title = "点击编辑";
...
function cancelEditMessage() {
  ...
  for (let var19 of var18) {
    var19.innerHTML = Zotero.AI4Paper.svg_icon_20px.edit;
    var19.title = "点击编辑";
