...
methodsBody.init = function () {
  ...
  document.getElementById("zotero-selectRefs-intro").textContent = "选择要导入的参考文献：0/" + methodsBody.io.dataIn.data.length;
...
methodsBody.buildItemNodes = function (param1) {
  ...
  for (var var5 in param1) {
    ...
    var3._hasRefsCache && Zotero.AI4Paper._isDuplicatedArray[var5] && Zotero.AI4Paper._isDuplicatedArray[var5]._isDuplicated === true && (var11.setAttribute("tooltiptext", "点击显示"), var11.innerHTML = Zotero.AI4Paper.svg_icon_16px.document_active, var11.setAttribute("_itemID", Zotero.AI4Paper._isDuplicatedArray[var5]._itemID), var11.addEventListener("mouseover", () => {
...
methodsBody.updateDuplicate = async function () {
  ...
  let var17 = (var15._references_DOI || '').split('🍋🎈🍋').filter(Boolean),
    var18;
  try {
...
methodsBody.selectAll = function (param3) {
  ...
  param3 ? document.getElementById("zotero-selectRefs-intro").textContent = "选择要导入的参考文献：0/" + methodsBody.io.dataIn.data.length : document.getElementById("zotero-selectRefs-intro").textContent = '选择要导入的参考文献：' + (methodsBody.io.dataIn.data.length - skipped) + '/' + methodsBody.io.dataIn.data.length;
};
...
methodsBody.handleCheckboxChange = function (param6) {
  ...
  if (var42 === "选择要导入的参考文献：0/" + methodsBody.io.dataIn.data.length) var41 && (document.getElementById("zotero-selectRefs-intro").textContent = "选择要导入的参考文献：1/" + methodsBody.io.dataIn.data.length);else {
    if (var41) {
    ...
    document.getElementById('zotero-selectRefs-intro').textContent = '选择要导入的参考文献：' + var45 + '/' + methodsBody.io.dataIn.data.length;
  }
};
