function init() {
  ...
  document.getElementById("ai4paper.customPublicationTitle.path").value = Zotero.Prefs.get("ai4paper.customPublicationTitlePath");
  document.getElementById("ai4paper.enableCustomPublicationTitle").checked = Zotero.Prefs.get("ai4paper.enableCustomPublicationTitle");
  document.getElementById("ai4paper.customJournalAbbr.path").value = Zotero.Prefs.get("ai4paper.customJournalAbbrPath");
  document.getElementById("ai4paper.enableCustomJournalAbbr").checked = Zotero.Prefs.get("ai4paper.enableCustomJournalAbbr");
...
async function choosePath_customPublicationTitle() {
  var {
    ...
    var1 = new _0x565c39();
  try {
    let var2 = Zotero.Prefs.get("ai4paper.customPublicationTitlePath");
  ...
  Zotero.Prefs.set("ai4paper.customPublicationTitlePath", var1.file);
  document.getElementById('ai4paper.customPublicationTitle.path').value = var1.file;
}
async function choosePath_customJournalAbbr() {
  var {
    ...
    var3 = new _0x4f7738();
  try {
    let var4 = Zotero.Prefs.get("ai4paper.customJournalAbbrPath");
    if (var4 && var4 != 'system') {
  ...
  Zotero.Prefs.set("ai4paper.customJournalAbbrPath", var3.file);
  document.getElementById('ai4paper.customJournalAbbr.path').value = var3.file;
}
async function importData_customPublicationTitle() {
  const var5 = document.querySelector("#ai4paper-customPublicationTitle-table-tbody");
  var5?.["querySelectorAll"]('tr')["forEach"](_0x57ce99 => _0x57ce99.remove());
  let var6 = Zotero.Prefs.get("ai4paper.customPublicationTitlePath");
  if (!var6) {
  ...
  if (!var7) return;
  Zotero.Prefs.set("ai4paper.customPublicationTitleData", JSON.stringify(var7));
...
async function importData_customJournalAbbr() {
  const var12 = document.querySelector("#ai4paper-customJournalAbbr-table-tbody");
  var12?.['querySelectorAll']('tr')["forEach"](_0x52385c => _0x52385c.remove());
  let var13 = Zotero.Prefs.get("ai4paper.customJournalAbbrPath");
  if (!var13) {
    window.alert("请先选择 CSV 文件路径！");
    return;
  }
  ...
  if (!var14) return;
  Zotero.Prefs.set("ai4paper.customJournalAbbrData", JSON.stringify(var14));
...
function updateTable_customPublicationTitle() {
  if (!Zotero.Prefs.get("ai4paper.customPublicationTitleData") || Zotero.Prefs.get('ai4paper.customPublicationTitleData') === '{}') return;
  const var26 = document.querySelector('#ai4paper-customPublicationTitle-table-tbody');
  var26?.['querySelectorAll']('tr')["forEach"](_0x366932 => _0x366932.remove());
  let var27 = JSON.parse(Zotero.Prefs.get("ai4paper.customPublicationTitleData"));
...
function updateTable_customJournalAbbr() {
  if (!Zotero.Prefs.get('ai4paper.customJournalAbbrData') || Zotero.Prefs.get("ai4paper.customJournalAbbrData") === '{}') return;
  const var32 = document.querySelector("#ai4paper-customJournalAbbr-table-tbody");
  var32?.["querySelectorAll"]('tr')["forEach"](_0x293a26 => _0x293a26.remove());
  let var33 = JSON.parse(Zotero.Prefs.get("ai4paper.customJournalAbbrData"));
