...
type JournalRankingRecord = {
  journal_name: string;
  issn: string;
  eissn: string;
  indexed_science: number;
  indexed_ssci: number;
  jcr_quartile: string;
...
var AI4PaperJournalRanking = (() => {
  ...
  function lookupCompactRecord(journalName: string | null | undefined, issn: string | null | undefined, eissn?: string | null): JournalRankingRecord | null {
  ...
  function lookupChineseIndex(journalName: string | null | undefined): string[] {
    ...
    if (cscd && cscd[normTitle]) tags.push(cscd[normTitle]);
    if (njucore && njucore[normTitle]) tags.push("CSSCI");
    if (pkucore && pkucore[normTitle]) tags.push("北核");

    return tags;
  }

  function lookupCCF(journalName: string | null | undefined): string | null {
    const normTitle = normalizeKey(journalName);
    if (!normTitle) return null;
  ...
  function getJCRColor(quartile: string): { bg: string; fg: string } {
    if (quartile === "Q1") return COLORS.jcrQ1;
    if (quartile === "Q2") return COLORS.jcrQ2;
    if (quartile === "Q3") return COLORS.jcrQ3;
    return COLORS.jcrQ4;
  }

  function getCASColor(zone: number, top: boolean): { bg: string; fg: string } {
    if (top) return COLORS.casTop;
    if (zone === 1) return COLORS.cas1;
    if (zone === 2) return COLORS.cas2;
    if (zone === 3) return COLORS.cas3;
    return COLORS.cas4;
  }

  function getIFColor(ifValue: number): { bg: string; fg: string } {
    if (ifValue >= 10) return COLORS.ifHigh;
    if (ifValue >= 3) return COLORS.ifMed;
    return COLORS.ifLow;
  }

  function getCCFColor(ccf: string): { bg: string; fg: string } {
    if (ccf.includes("A")) return COLORS.ccfA;
    if (ccf.includes("B")) return COLORS.ccfB;
    return COLORS.ccfC;
  }

  function getChineseIndexColor(tag: string): { bg: string; fg: string } {
    if (tag.includes("CSCD")) return COLORS.cscd;
    if (tag === "CSSCI") return COLORS.cssci;
    if (tag === "北核") return COLORS.pkucore;
    return COLORS.fallback;
  }

  function buildBadges(item: any): Badge[] {
    try {
  ...
  function buildTextSummary(item: any): string {
    ...
    try {
      ...
      const parts: string[] = [];
      if (libCat && libCat !== "0") parts.push(libCat);
      if (callNum && callNum !== "0") parts.push(callNum);
      return parts.join(" · ");
    } catch (_e) {
      return "";
    }
  ...
  async function registerColumns(): Promise<void> {
    ...
    await Zotero.ItemTreeManager.registerColumn({
      ...
      dataProvider: (item: any, _dataKey: string) => {
        const badges = buildBadges(item);
        if (badges.length === 0) {
          // Fallback text
          try {
            const libCat = String(item.getField("libraryCatalog") ?? "").trim();
            if (libCat && libCat !== "0") return libCat;
          } catch (_e) { /* */ }
          return "";
        }
  ...
  async function unregisterColumns(): Promise<void> {
    try {
